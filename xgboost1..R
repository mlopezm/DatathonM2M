...
if(h2_1==2){
  ...
  load("sim.uwgh2_4.all.new.Rdat")  # tiene intervalos de 4 horas y resuelve el problema de imputacion de 2h al principio
  }else {
  #   sim.ids=sample(valsims, 200, replace=F)
...
# historyrunsm
# historyrunssd
# save (historyruns,file="hr_xgboost1_all_2h_4hd_ts.0.7tail.Rdat")
# 
...
# save(sample_good,sample_bad,file="./sample_gb/samp_badgood_xgb12.Rdat")
...
for (sid in sim.ids){
  ...
  y = as.numeric(train_tr[,"hit"])
  x1= train_tr[ ,c(3,4,7,8)]  #nos quedamos con wd, hd, h2d, h4d
  x2= train_te [ ,c(3,4,7,8)]
  x = rbind(x1,x2)
  x = as.matrix(x)
  x = matrix(as.numeric(x),nrow(x),ncol(x))
  trind = 1:length(y)
  teind = (nrow(train_tr)+1):nrow(x)
  ...
  if (sum(train_tr$hit!="0")==0 | sum(train_tr$hit!="1")==0){ # si todos son 0 o 1
    ...
    if(h2_1==2){
      ...
      bst.cv = xgb.cv(param=param, data = x[trind,], label = y, 
                      nfold =10, nrounds=nr,verbose=F)
      t.m=as.numeric(bst.cv$test.rmse.mean)
      wtm=which(t.m==min(t.m))
      bst = xgboost(param=param, data = x[trind,], label = y, nrounds=wtm,verbose=0)
    } else {
      bst.cv = xgb.cv(param=param, data = x[trind,], label = y, 
                      nfold = 10, nrounds=nr,verbose=F)
      t.m=as.numeric(bst.cv$test.rmse.mean)
...
historyrunsm
historyrunssd
save (historyruns,file="hr_xgboost1_all_2h_4hd_ts.reto.Rdat")
...
#guardar las peores y mejores muestras
sample_good=hr$simid[hr$loss==1]
sample_bad=hr$simid[hr$loss==-1]
save(sample_good,sample_bad,file="./sample_gb/samp_badgood_xgb13.Rdat")
