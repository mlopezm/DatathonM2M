...
for (account in accounts){
  rplans=unique((sim.uwg %>% filter(acc==account))$rplan)
  for (rplan in rplans){
    tt= sim.uwg %>% filter(acc==account,rplan==rplan)
    vallossf=NULL
    count=count+1
    print(count)
    ...
    historyruns=rbind(historyruns,c( account=account,
...
historyrunsm
historyrunssd
save (historyruns,file="hr_rf7_all_2h_4hd_ts0.7tail.Rdat")
...
save(sample_good,sample_bad,file="./sample_gb/samp_badgood_rf72.Rdat")
